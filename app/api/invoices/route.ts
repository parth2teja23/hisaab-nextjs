...
export async function POST(req: Request) {
  try {
    ...
    // Auto-link customer to this store
    ...
    const productIds = items.map((i: InvoiceItemInput) => i.productId);
    ...
    // if the email takes a few seconds or fails.
    if (customer.email) {
      // Don't await this if you want the API to be fast
      sendInvoiceEmail(customer.email, invoice).catch((err) => 
        console.error("Background email failed:", err)
      );
    } else {
      console.log("No email found for customer, skipping invoice email.");
    }
